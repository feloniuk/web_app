...
def edit_publication(request, slug):
    try:
        ...
        return HttpResponseNotFound(f'Publication id {slug} doesnt exist')
    if request.method == 'POST':
        form = PublicationEditForm(request.POST, instance=publication)
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(f'/publications')
